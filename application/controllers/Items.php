...
class Items extends CI_Controller{
    ...
    */
    ...
    public function add(){
        ...
        $this->form_validation->set_rules('itemBasePrice', 'Item Base Price', ['required', 'trim', 'numeric'], ['required'=>"required"]);
        ...
        if($this->form_validation->run() !== FALSE){
             ...
             */
            $insertedId = $this->item->add(set_value('itemName'), set_value('itemQuantity'), set_value('itemPrice'), set_value('itemBasePrice'), 
                    set_value('itemDescription'), set_value('itemCode'));
            
            $itemName = set_value('itemName');
            $itemQty = set_value('itemQuantity');
            $itemPrice = "P ".number_format(set_value('itemPrice'), 2);
            $itemBasePrice = "P ".number_format(set_value('itemBasePrice'), 2);
            
            //insert into eventlog
            //function header: addevent($event, $eventRowId, $eventDesc, $eventTable, $staffId)
            $desc = "Addition of {$itemQty} quantities of a new item '{$itemName}' with a selling price of {$itemPrice} to stock";
    ...
    */
    ...
    public function edit(){
        ...
        $this->form_validation->set_rules('itemBasePrice', 'Item Unit Price', ['required', 'trim', 'numeric']);
        ...
        if($this->form_validation->run() !== FALSE){
            ...
            $itemBasePrice = set_value('itemBasePrice');
            $itemName = set_value('itemName');
            $itemCode = $this->input->post('itemCode', TRUE);
            
            //update item in db
            $updated = $this->item->edit($itemId, $itemName, $itemDesc, $itemPrice, $itemBasePrice);
